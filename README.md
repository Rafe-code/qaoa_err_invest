...
<!-- Talk explicitly about error mitigation techniques, which are not considered here, but may have angle dependence and therefore introduce angle dependent error rates -->
...
<!-- rewrite now that the parameters have been taken above -->
...
When the `Rzz` investigation circuit, given in [Figure 3](#fig-3) is evaluated with the input `|00⟩`, the anticipated result, `|φ⟩`, is an equal superposition between all possible output states, given by
...
<!-- TODO: good reason why simulating ideal for Rx and theory for Rzz (it was easier to implement may be enough...). Why I chose the E_rel as the metric to go with, would fidelity have done the same thing? -->
...
<!-- TODO: check axis and titles etc, statistical test, double check angle range (doubling the angle going in) -->
...
The lack of strong evidence for a relationship between angle and fidelity in [Figure 4](#fig-4) suggests that angle offsets are unlikely to minimize overall error rates. Furthermore, due to the mirrored responses, improvements to the |00⟩ channel's error rate would likely be offset by deteriorations in the |10⟩ channel, and vice versa.

### Rx Gate

<!-- TODO: sharpen up the numbers used, check axis and titles etc, talk about reproducibility between each run (small error bars) -->

The results of the `Rx` gate investigation can be seen in [Figure 5](#fig-5).
